...
#発射元ターゲット
execute as @s[scores={death_timer=1}] at @s positioned ~~~ run tag @e[c=1,family=!inanimate,type=!item,tag=!E_assr] add E_assr_settinger
#初期位置
...
#ヒット時ダメージ*最後に持ってくること*
execute as @s at @s positioned ~~~ as @e[tag=!E_assr,family=!inanimate,type=!item] if score @s UUID = @e[tag=E_assr,c=1] UUID run damage @e[tag=E_assr_hitter] 2 entity_attack entity @s
#ヒット処理終了
